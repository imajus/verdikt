...
- **Arc Testnet** — `VerdiktRegistry` at `0xE182626142E63EF440421cb0c5e4DEbeEF76E4Af`, block 60860488. As of 2026-09-13 ten services are bonded and `ACTIVE` (`portfolio`, `pnl`, `flights`, `enrich`, `product`, `domain`, `people`, `companies`, `reddit`, `prices`), registered self-serve through the dashboard's wizard between 2026-09-11 and 2026-09-13, each fronting a real third-party x402 provider (Alchemy, Allium, Syntalic, …). The registry holds 44 verdicts and 23 credited refunds, most from real paid calls with real payers — `.claude/skills/verdikt-paid-call-sweep` is how that traffic is generated and read back. The original demo pair `weather` / `weather-lite`, which `docs/walkthrough.md` and the evidence transcripts describe, was deregistered on 2026-09-11; the registry marks both `DEREGISTERED` and neither slug can be re-registered on this deployment. A retired service is delisted, so both the dashboard (`isListed` in `web/src/marketplace.js`) and the aggregate (`reputationForWindow` in `cre/lib/reputation.js`) drop it; their detail pages stay reachable by slug because the verdict history is permanent. **Do not take the service list from this file** — it changes daily; read it off the chain with the sweep skill's `list-services.mjs`.
...
All three spikes have run. **A** (ENSv2) — `docs/spikes/A-ens-sepolia.md`. **B** (CRE) — `docs/spikes/cre.md`, findings CRE-1…CRE-10, several of which reshaped the contracts. **C** (payment header) — `docs/spikes/C-x402-payment.md`. Both `exact` transfer methods a real challenge offers verify through one ERC-3009 verifier: plain `eip3009` (a signature it produced settled on Base Sepolia) and Circle's `GatewayWalletBatched` (issue #41 — refused as unpublished until 2026-09-11, then recovered against a real captured header). A contract-account payer such as the Circle agent wallet is asked via ERC-1271 over the payment chain's RPC (`PAYMENT_<NAME>_RPC_URL`, #55). A genuinely unknown scheme still refuses, per header.

Two things are deliberately unfinished, each argued at its own task in `docs/Tasks.md`: production CRE enrollment (`cre whoami` → *Deploy Access: Not enabled*) and the recorded video. The paid call end to end, long blocked on the original demo paywall, is no longer: real agents pay real providers through `<slug>.verdikt.bond`, the verify workflow judges the call, and the verdict and refund land on Arc. One gap remains on that leg — a Gateway-paid refund is booked to the agent wallet's *backing EOA*, which cannot call `withdraw()` on Arc; `withdrawWithAuthorization` closes it in the contract, but the live registry predates it and needs a redeploy (Tasks.md 2.4).
