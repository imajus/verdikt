...
- **Arc Testnet** — `VerdiktRegistry` at `0xE182626142E63EF440421cb0c5e4DEbeEF76E4Af`, block 60860488. Two services bonded, three verdicts written through the real KeystoneForwarder, refunds credited and withdrawn — the evidence transcripts capture that period. `weather` and `weather-lite` were deregistered on 2026-09-11 (bonds returned to the provider wallet); the registry marks both `DEREGISTERED` and neither slug can be re-registered on this deployment. **Live mode therefore shows an empty marketplace** — a retired service is delisted, so both the dashboard (`isListed` in `web/src/marketplace.js`) and the aggregate (`reputationForWindow` in `cre/lib/reputation.js`) drop it; their detail pages stay reachable by slug because the verdict history is permanent. A recording per `docs/shot-list.md` now needs two freshly registered services first.
