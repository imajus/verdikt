...
A marketplace of x402-gated API services whose delivery is verified per call. A proxy sits between a paying agent and an x402 provider, verifies the response against the provider's own declared SLA inside a Chainlink CRE Confidential Workflow, writes a verdict on-chain, and auto-refunds from the provider's bonded deposit when the SLA isn't met. That verdict is final: no challenge, no arbiter, by design.

What determinism cannot judge — whether the content *meant* what was promised — is a second, independent judgment: a consumer files a bonded claim against a `semantic` clause on GenLayer, and validators judge it under Optimistic Democracy. It is **not an appeal** of the CRE verdict. The two answer different questions over different traffic, are stored and scored separately, and a call can legitimately be CRE PASS and semantically BREACH — which is the only fact worth showing.
...
- **GenLayer Studio Devnet** (chain 61997) — `SlaClaimJudge` and `SettlementToken`, addresses in `deployments/genlayer-studio-devnet.json`, which every reader on both sides of the language boundary takes them from rather than hand-copying. Two claims have resolved against `aisa`'s `meets-criteria` clause — one `BREACH`, one `MET` — read back with `genlayer/scripts/export-claims.py`. It is a hosted release-candidate simulator whose state GenLayer does not guarantee across deployments; expect to redeploy. `semanticConformance` is computed by `scripts/publish-semantic-scores.mjs` but **published nowhere yet**: every live subname was minted before the key existed, so the first write reverts until `--grant` runs with the operator key, and `aisa` reads back `null` today.
- **Two payer gates are disabled for the demo**, greppable as `TEMPORARY (hackathon demo)` and to be restored together: `check_eligibility` no longer requires a claimant to be the payer, and the proxy no longer requires a payer signature to disclose evidence. Six JS tests and one Python test are skipped, not deleted. The proxy half is the exposure that matters — deployed, `/internal/evidence/<request_id>` serves any paid response body to anyone holding a request id, and request ids are public in every `VerdictWritten` event. They came off because `aisa` is the only registered service with a `semantic` clause and is payable only via `GatewayWalletBatched`, so its verdicts book a backing EOA Circle will not sign as and that cannot transact on GenLayer — no disputable call existed that a key we hold could have paid for.
...
**Ethereum Sepolia** holds ENS. The SLA lives *only* as the `sla` text record on `<slug>.verdikt.eth` — there is no SLA field on Arc and no `setSLA`. ENSv2's Permissioned Resolver is chosen over v1 specifically for per-key access control: the provider is scoped to write `sla`, the CRE signer to `conformance`/`availability`, the semantic aggregator to `semanticConformance`, and cross-writes revert. Spike A confirmed that ACL enforces on Sepolia, and also that it has two bypasses the role bitmap must close: never grant a provider `ROLE_SET_RESOLVER` on its own subname, and grant per-key with `authorizeTextRoles`, never name-wide with `authorizeNameRoles`.

**GenLayer** holds the semantic claim, and settles it in its own token rather than relaying a judgment back to Arc. An earlier design did relay it, and that bought a cross-chain trust model, a relay to run and fund, and finality coupling between two chains with unrelated appeal semantics — for a requirement that was never real: a claimant needs the judgment to have a consequence the provider feels, not one denominated in the asset the call was paid in. GenLayer still *reads* Arc (`getVerdict`, hand-rolled calldata, one batched JSON-RPC POST) to bind a claim to a verdict, and a read is not a bridge — nothing on Arc has to trust anything on GenLayer. `docs/GenLayer.md` is the argument in full.
...
- **A semantic outcome never merges into `conformance`.** `semanticConformance` is its own score, written by its own signer, over its own denominator: only `MET` and `BREACH` count, because `OPEN` has not decided and `UNDETERMINED`/`CANCELLED` decided nothing — the same reasoning that keeps `DOWN` out of the conformance denominator. Averaging the two would hide the one case the GenLayer leg exists to surface: a spotless deterministic record next to a poor semantic one. Empty scores 1000, like the other two.
...
- **`web/src/genlayer.js` is the only file that *talks to* GenLayer**, and it lives in `web/` rather than the SDK because the SDK is imported by the proxy and the CRE workflow, neither of which has any business reading GenLayer. `null` (no judge configured, or unreadable) and `[]` (read fine, nothing disputed) are different answers and must render differently. What the rule protects is *protocol* knowledge — the `gen_call` calldata codec, what a claim or a clause is, how to judge one — not the word "GenLayer" appearing in a file. So the proxy may relay the judge's address and chain id to a disputing agent as response headers ([#114](https://github.com/imajus/verdikt/issues/114)): opaque identifiers, read out of `deployments/genlayer-studio-devnet.json` like every other deployed address and never hand-copied into `wrangler.jsonc`, with no GenLayer import and no GenLayer call behind them. The argument is at the top of `proxy/src/config.js`. The line is decoding: anything that needs the proxy to *interpret* a GenLayer value rather than pass it through is over it.
