 ...
 * an empty promise rather than silently claiming to be a schema check.
 *
...
const clausePromise = (clause) => {
  switch (clause.type) {
    case 'latency':
      return `Answers within ${duration(clause.maxMs)}`;
    case 'priceRange':
      return `Priced from ${formatMinorRange(BigInt(clause.minMinorUnits), BigInt(clause.maxMinorUnits))} a call`;
    case 'schema':
...
const how = () => html`
  ...
  <section class="block"><h3>Three chains, each for one reason</h3><p><strong>Arc</strong> holds the registry, the escrow, the verdicts and the refunds — and the x402 payment itself. USDC is Arc's native gas token, so value moves as <code>msg.value</code>, not an ERC-20 transfer: no <code>approve</code>/<code>transferFrom</code>, no token address. Payment, bond and refund are the same asset on the same chain, which removes any cross-chain correlation between payment and refund.</p><p><strong>Ethereum Sepolia</strong> holds ENS. The SLA lives only as the <code>sla</code> text record on <code>&lt;slug&gt;.verdikt.eth</code>. A per-key access list scopes the provider to <code>sla</code> and <code>url</code>, the CRE signer to <code>conformance</code> and <code>availability</code>, and the semantic aggregator to <code>semanticConformance</code>. Cross-writes revert.</p><p><strong>GenLayer</strong> holds the semantic claim, described below, and settles it in its own token rather than reaching back into Arc's escrow.</p></section>
  <section class="block"><h3>What happens on a paid call</h3><p>A proxy sits between the paying agent and the provider's x402 endpoint. Without a payment header, the provider's own 402 challenge is relayed back unchanged — the proxy verifies nothing about it, because the trust anchor is the service's registered <code>url</code>, bound to its bond, and an upstream that writes the challenge can name whatever payout address it likes. With payment attached, the call is replayed inside a Chainlink CRE Confidential Workflow and evaluated against the provider's published SLA without exposing the raw response outside the enclave.</p><p>The workflow writes PASS, FAIL or DOWN on Arc. A FAIL or DOWN credits the payer from the service's bond, capped at <code>min(fixed refund, what was actually paid, what remains of the bond)</code> — booked, not sent. The payer collects it by connecting the credited wallet on <a href=${WITHDRAW_PATH}>the withdraw page</a>.</p></section>
  <section class="block"><h3>The verdict is final — and that is not the whole question</h3><p>There is no challenge process and no arbiter. The refund cap keeps a false FAIL from being worth manufacturing, the observed value never goes on-chain, and the clause, refund and trailing one-day scores stay public on Arc and ENS.</p><p>But a verdict is a deterministic function, and that bounds what it can say. It knows the response parsed, arrived in time and was priced as advertised. It cannot know the summary summarised the wrong document.</p></section>
  <section class="block"><h3>Disputing what the answer meant</h3><p>So a provider may also publish a <strong>semantic</strong> clause: a plain-English promise about what the response says, binding but impossible to check deterministically. A consumer who thinks it was broken posts a bond and files a claim on GenLayer, naming the call and the clause. The claim binds to the verdict already on Arc, so a call nobody paid for cannot be disputed.</p><p>The evidence is the request and the response together — a body alone cannot be judged, because the promise is about whether <em>this</em> request was answered. It is disclosed only to the agent that paid for it, and the criteria are frozen when the claim opens, so a provider cannot edit its SLA mid-dispute. Validators then judge it independently under Optimistic Democracy and compare outcomes, rather than checking that a leader's answer was formatted correctly.</p><p>Four outcomes. <strong>BREACH</strong> compensates the consumer from the provider's GenLayer deposit. <strong>MET</strong> costs the consumer the bounty out of its own bond — filing is not free. <strong>UNDETERMINED</strong> and <strong>CANCELLED</strong> charge nobody: where the evidence cannot honestly settle the question, forcing a binary answer would only make missing evidence worth manufacturing.</p><p>This is a second judgment, <strong>not an appeal</strong>. It asks a different question of different traffic, so it is scored as its own figure and never averaged into conformance — a service can hold a spotless deterministic record and a poor semantic one, and that contrast is the thing worth seeing.</p></section>`;
