...
class FilterTypeNotSupportedError extends BaseError3 {
  constructor(type) {
...
var NO_DATA_SCORE = SCORE_SCALE;
function aggregateWindow(verdicts) {
  let pass = 0;
  let fail = 0;
  let down = 0;
...
function reputationForWindow({ services, verdicts, now, windowSeconds = WINDOW_SECONDS }) {
  if (!Number.isFinite(now))
    throw new Error("reputationForWindow: `now` must be a Unix timestamp in seconds");
  ...
  for (const verdict of verdicts) {
    if (verdict.timestamp <= since || verdict.timestamp > now)
      continue;
    const bucket = byService.get(verdict.serviceId);
    if (bucket)
      bucket.push(verdict.outcome);
  }
  return services.filter((service) => byService.has(service.serviceId)).map((service) => ({
    serviceId: service.serviceId,
    slug: service.slug,
    ...aggregateWindow(byService.get(service.serviceId))
  }));
}
...
var OUTCOME_BY_ORDINAL = ["PASS", "FAIL", "DOWN"];
var STATUS_BY_ORDINAL = ["NONE", "ACTIVE", "SUSPENDED", "DEREGISTERED"];
function outcomeFromOrdinal(ordinal) {
...
function statusFromOrdinal(ordinal) {
...
var registryViews = parseAbi([
  "function getService(bytes32 serviceId) view returns ((address provider, uint8 status, uint256 deposit))"
]);
var SCORE_REPORT_PARAMS = parseAbiParameters("string slug, uint256 conformance, uint256 availability");
var bigIntJson = (value2) => ({
  absVal: hexToBase64(numberToHex(value2 < 0n ? -value2 : value2, { size: 32 })),
  sign: value2 < 0n ? "-1" : "1"
});
var topicOf = (eventName) => encodeEventTopics({ abi: registryEvents, eventName })[0];
var onSchedule = (runtime2, _trigger) => {
  ...
  if (!sepolia)
    throw new Error(`unknown chain selector name: ${config.sepoliaChainSelectorName}`);
  const arcClient = new ClientCapability(arc.chainSelector.selector);
  const registry2 = hexToBase64(config.registryAddress);
  const deployBlock = BigInt(config.registryDeployBlock);
  const head = arcClient.headerByNumber(runtime2, { blockNumber: bigIntJson(-1n) }).result().header;
  if (!head?.blockNumber)
    throw new Error("could not read Arc head");
  const headNumber = BigInt(bytesToHex3(head.blockNumber.absVal));
  ...
  const timestampOfBlock = (blockNumber) => headTimestamp - Number(headNumber - blockNumber) * config.blockTimeSeconds;
  const registrationLogs = arcClient.filterLogs(runtime2, {
    filterQuery: {
  ...
  const services = registrationLogs.map((log) => {
    const decoded = decodeEventLog({
      ...
      data: bytesToHex3(log.data),
      topics: log.topics.map(bytesToHex3)
    });
    const serviceId = decoded.args.serviceId;
    const reply = arcClient.callContract(runtime2, {
      call: {
        to: registry2,
  ...
  const verdicts = verdictLogs.map((log) => {
    ...
    return {
      serviceId: decoded.args.serviceId,
      outcome: outcomeFromOrdinal(Number(decoded.args.outcome)),
      timestamp: timestampOfBlock(log.blockNumber ? BigInt(bytesToHex3(log.blockNumber.absVal)) : headNumber)
    };
  });
  const scores = reputationForWindow({ services, verdicts, now: headTimestamp, windowSeconds: WINDOW_SECONDS });
  const published = [];
  for (const score of scores) {
    const report2 = runtime2.report(prepareReportRequest(encodeAbiParameters(SCORE_REPORT_PARAMS, [score.slug, BigInt(score.conformance), BigInt(score.availability)]))).result();
    const txResult = new ClientCapability(sepolia.chainSelector.selector).writeReport(runtime2, {
