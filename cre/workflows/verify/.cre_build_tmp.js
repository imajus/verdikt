...
var ANNOTATION_KEYWORDS = new Set(["$schema", "$id", "$comment", "title", "description", "examples", "default"]);
...
var NON_NEGATIVE_INTEGER_KEYWORDS = ["minItems", "maxItems", "minLength", "maxLength"];
var FINITE_NUMBER_KEYWORDS = ["minimum", "maximum"];
var isPlainObject = (value2) => value2 !== null && typeof value2 === "object" && !Array.isArray(value2);
var escapeToken = (token) => token.replaceAll("~", "~0").replaceAll("/", "~1");
var child = (pointer, token) => `${pointer}/${escapeToken(String(token))}`;
function assertSchema(schema, pointer = "") {
  ...
  if ("type" in schema) {
    ...
    if (names.length === 0)
      throw new Error(`sla: "type" at ${at} must name at least one type`);
  ...
  if ("properties" in schema) {
    if (!isPlainObject(schema.properties))
      throw new Error(`sla: "properties" at ${at} must be an object`);
  ...
  if ("items" in schema)
    assertSchema(schema.items, `${pointer}/items`);
  if ("oneOf" in schema) {
...
function matchesType(value2, name) {
  switch (name) {
    case "null":
      return value2 === null;
    case "array":
...
function describeValue(value2) {
  ...
  switch (typeof value2) {
    case "object":
      return `object{${Object.keys(value2).length}}`;
    case "string":
      return value2.length > 40 ? `string ${JSON.stringify(value2.slice(0, 40))}…` : `string ${JSON.stringify(value2)}`;
    case "number":
...
function deepEqual(a, b) {
  ...
  if (isPlainObject(a) && isPlainObject(b)) {
    ...
    return keys.length === otherKeys.length && keys.every((key, i2) => key === otherKeys[i2] && deepEqual(a[key], b[key]));
  }
  return false;
}
var fail = (pointer, expected, actual) => ({ pointer, expected, actual });
function validate(schema, value2, pointer = "") {
  if (!isPlainObject(schema))
    throw new Error("sla: validate() called with a non-object schema");
  if ("type" in schema) {
    const names = Array.isArray(schema.type) ? schema.type : [schema.type];
    if (!names.some((name) => matchesType(value2, name))) {
      return fail(pointer, names.join(" | "), describeValue(value2));
    }
  }
  if ("const" in schema && !deepEqual(value2, schema.const)) {
    return fail(pointer, JSON.stringify(schema.const) ?? "undefined", describeValue(value2));
  }
  if ("enum" in schema) {
    const options = schema.enum;
    if (!options.some((option) => deepEqual(value2, option))) {
      return fail(pointer, `one of ${JSON.stringify(options)}`, describeValue(value2));
    }
  }
  if (typeof value2 === "number") {
    if (typeof schema.minimum === "number" && value2 < schema.minimum) {
      return fail(pointer, `>= ${schema.minimum}`, String(value2));
    }
    if (typeof schema.maximum === "number" && value2 > schema.maximum) {
      return fail(pointer, `<= ${schema.maximum}`, String(value2));
    }
  }
  if (typeof value2 === "string") {
    if (typeof schema.minLength === "number" && value2.length < schema.minLength) {
      return fail(pointer, `length >= ${schema.minLength}`, `length ${value2.length}`);
    }
    if (typeof schema.maxLength === "number" && value2.length > schema.maxLength) {
      return fail(pointer, `length <= ${schema.maxLength}`, `length ${value2.length}`);
    }
  }
  if (Array.isArray(value2)) {
    if (typeof schema.minItems === "number" && value2.length < schema.minItems) {
      return fail(pointer, `>= ${schema.minItems} items`, `${value2.length} items`);
    }
    if (typeof schema.maxItems === "number" && value2.length > schema.maxItems) {
      return fail(pointer, `<= ${schema.maxItems} items`, `${value2.length} items`);
    }
    if ("items" in schema) {
      for (let index = 0;index < value2.length; index += 1) {
        const failure = validate(schema.items, value2[index], child(pointer, index));
        if (failure)
          return failure;
      }
    }
  ...
  if (isPlainObject(value2)) {
    if (Array.isArray(schema.required)) {
      for (const name of schema.required) {
        if (!Object.hasOwn(value2, name)) {
          return fail(child(pointer, name), "present", "absent");
        }
      }
    }
    ...
    for (const name of Object.keys(properties).sort()) {
      if (!Object.hasOwn(value2, name))
        continue;
      const failure = validate(properties[name], value2[name], child(pointer, name));
      if (failure)
        return failure;
    }
    if ("additionalProperties" in schema) {
      for (const name of Object.keys(value2).sort()) {
        if (Object.hasOwn(properties, name))
          continue;
        if (schema.additionalProperties === false) {
          return fail(child(pointer, name), "no such member", describeValue(value2[name]));
        }
        if (schema.additionalProperties !== true) {
          const failure = validate(schema.additionalProperties, value2[name], child(pointer, name));
          if (failure)
            return failure;
        }
      }
  ...
  if (Array.isArray(schema.oneOf)) {
    const matches = schema.oneOf.filter((branch) => validate(branch, value2, pointer) === null);
    if (matches.length !== 1) {
...
var DELIVERY_CLAUSE_ID = "delivery";
var display = (pointer) => pointer === "" ? "(root)" : pointer;
function evaluateDelivery(observation) {
...
function evaluateClause(clause, observation) {
  switch (clause.type) {
    case "schema": {
    ...
    default:
      throw new Error(`sla: unknown clause type "${clause.type}"`);
  }
}
var schema_default = {
...
function assertObservation(observation) {
  ...
  const { status, headers, latencyMs, paidAmount } = observation;
...
function assertSlaDocument(sla) {
  const failure = validate(schema_default, sla);
  if (failure) {
...
function parseSla(text2) {
  if (typeof text2 !== "string")
    throw new Error("sla: parseSla() expects the raw text record as a string");
  const parsed = JSON.parse(text2);
  assertSlaDocument(parsed);
  return parsed;
}
function evaluate(sla, observation) {
...
function evaluateStatusOnly(observation) {
  assertObservation(observation);
  const { status } = observation;
  if (status === null)
    return { outcome: OUTCOME.DOWN, clauses: [] };
  if (status >= 200 && status < 300)
    return { outcome: OUTCOME.PASS, clauses: [] };
  if (status >= 500)
    return { outcome: OUTCOME.FAIL, clauses: [] };
  return { outcome: null, clauses: [] };
}
var JUDGEMENT_MODE = Object.freeze({
  SLA: "sla",
  STATUS_ONLY: "status-only"
});
function judge(slaText, observation) {
  if (typeof slaText === "string" && slaText.length > 0) {
    try {
      ...
      return {
        ...
        fallbackReason: `sla unusable: ${error2.message}`
      };
    }
  }
  return {
...
var shouldWriteVerdict = (judgement) => judgement.outcome !== null;
function observationFrom(seen) {
  let body = null;
...
var OUTCOME_ORDINAL = Object.freeze({
  PASS: 0,
  FAIL: 1,
  DOWN: 2
});
var STATUS_ORDINAL = Object.freeze({
  NONE: 0,
  ACTIVE: 1,
  SUSPENDED: 2,
  DEREGISTERED: 3
});
function outcomeToOrdinal(outcome) {
  if (!Object.hasOwn(OUTCOME_ORDINAL, outcome))
    throw new Error(`unknown outcome: ${outcome}`);
  return OUTCOME_ORDINAL[outcome];
}
var VERDICT_REPORT_PARAMS = parseAbiParameters("bytes32 serviceId, bytes32 requestId, uint8 outcome, address payer, uint256 paidAmount");
var onVerifyRequest = (runtime2, trigger) => {
  const config = runtime2.config;
  const request = decodeJson(trigger.input);
  const network295 = getNetwork({ chainFamily: "evm", chainSelectorName: config.arcChainSelectorName });
  if (!network295)
    throw new Error(`unknown chain selector name: ${config.arcChainSelectorName}`);
  const started = runtime2.now().getTime();
  let status = null;
  let bodyText;
  let transportError;
  try {
    const response = new ClientCapability2().sendRequest(runtime2, {
  ...
  const latencyMs = runtime2.now().getTime() - started;
  const judgement = judge(request.sla, observationFrom({
    status,
    bodyText,
    latencyMs,
  ...
  const encodedVerdict = encodeAbiParameters(VERDICT_REPORT_PARAMS, [
    request.serviceId,
    request.requestId,
    outcomeToOrdinal(judgement.outcome),
    request.payer,
    BigInt(request.paidAmountMinorUnits)
  ]);
  const signedReport = donRuntime.report(prepareReportRequest(encodedVerdict)).result();
  const txResult = new ClientCapability(network295.chainSelector.selector).writeReport(donRuntime, {
  ...
  return JSON.stringify({
    ...
    tx: bytesToHex3(txResult.txHash ?? new Uint8Array(32))
  });
};
var initWorkflow = (config) => {
  const http2 = new HTTPCapability;
  return [
    handlerInTee(http2.trigger({ authorizedKeys: config.authorizedKeys }), onVerifyRequest, [{ tee: "nitro", regions: ["us-west-2"] }])
  ];
};
async function main() {
